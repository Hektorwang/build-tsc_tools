...
# =============================================================================
# lib/raid/sas2.sh — SAS2 (mpt2sas) RAID 卡适配器
# =============================================================================
# 被 source 使用，不可直接执行。
#
# 支持的控制器: LSI SAS2008、SAS2308 等 mpt2sas 驱动的 HBA/RAID 卡
# 管理工具: sas2ircu
#
# 依赖:
...
#     遍历所有 SAS2 控制器，采集 IR Volume 和物理磁盘状态
#     结果追加到全局变量 raid_status_json（调用方需先初始化为 "[]"）
#
# 注意: SAS2 与 SAS3 的 sas2ircu/sas3ircu 命令输出格式相同，
#       本文件逻辑与 sas3.sh 完全一致，仅工具名不同
# =============================================================================
...
#
# 参数:
#   raid_bin — sas2ircu 的完整路径
#
# 副作用:
#   追加到全局变量 raid_status_json（JSON 数组）
# -----------------------------------------------------------------------------
raid_check_health_sas2() {
    local RAID_BIN="$1"
    local ctls=() ctl_no

    # 获取所有控制器编号（格式与 sas3ircu 相同）
    ...
    for ctl_no in "${ctls[@]}"; do
        ...
        # grep -A 13 "Device is a Hard disk" 取每个硬盘设备后13行（包含 Slot 和 State 字段）
